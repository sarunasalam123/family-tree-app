...
function drawAncestryTree(opts: {
  ...
  // Force-directed x-adjustment when showDuplicates=false and there are cross-links.
  // y-coordinates stay fixed (generational hierarchy preserved); force simulation
  // only shifts x so that cross-linked nodes end up closer together, reducing
  // long dashed-line tension across the tree.
  if (!showDuplicates && extraLinks.length > 0) {
    interface SimNode extends d3.SimulationNodeDatum {
      ...
      fx?: number | null;
      fy: number;
      hNode: d3.HierarchyNode<ApiNode>;
    }

    const allNodes = root.descendants();
    const simNodes: SimNode[] = allNodes.map((d) => ({
      id: `${d.data.type}:${d.data.id}`,
      x: d.x ?? 0,
      y: d.y ?? 0,
      fy: d.y ?? 0, // fix y so generation levels don't collapse
      hNode: d,
    }));

    // Fix the root node's x so both trees stay anchored to the same center axis
    if (simNodes.length > 0) simNodes[0].fx = simNodes[0].x;

    const idToSim = new Map(simNodes.map((n) => [n.id, n]));

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const simLinks: { source: any; target: any; strength: number }[] = [];

    // Hierarchy edges — kept weak so the tree can flex on x
    allNodes.forEach((d) => {
      if (!d.parent) return;
      const s = idToSim.get(`${d.parent.data.type}:${d.parent.data.id}`);
      const t = idToSim.get(`${d.data.type}:${d.data.id}`);
      if (s && t) simLinks.push({ source: s, target: t, strength: 0.1 });
    });

    // Cross-link edges — strong pull to bring linked nodes closer together
    extraLinks.forEach((link) => {
      const s = idToSim.get(`family:${link.from_fam}`);
      const t = idToSim.get(`person:${link.to_person}`);
      if (s && t) simLinks.push({ source: s, target: t, strength: 1.0 });
    });
    ...
    const simulation = d3
      ...
      .force("collide", d3.forceCollide<SimNode>(120))
      .stop();

    // Run synchronously for 300 ticks — enough to settle without visual delay
    for (let i = 0; i < 300; i++) simulation.tick();

    // Write simulated x back to the d3 hierarchy nodes
    simNodes.forEach((sn) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (sn.hNode as any).x = sn.x;
    });
  }
